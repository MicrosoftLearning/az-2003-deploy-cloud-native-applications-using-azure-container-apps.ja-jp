...
lab:
  ...
  description: この演習では、Azure Container Apps ソリューションをサポートする Azure リソースを構成します。
  duration: 15 minutes
  level: 500
  islab: true
  primarytopics:
...
1. **[AZ2003Project]** を選択します

1. [AZ2003Project] ページの左側のメニューで、**[リポジトリ]** を選択します。

1. [すべてのリポジトリ] で、**[AZ2003Project]** を選択し、**[参照]** を選択します。

1. **[Clone in VS Code](VS コードにクローンする)** を選択します。
...
1. レジストリに接続するには、Visual Studio Code で、Azure で適切に認証されている必要があります。

    - アクティビティ バーで **[Azure]** (Azure アイコン) を選択します。
    - Azure ビューで、**[アカウントとテナント]** を展開します。
    - お使いの Azure アカウントが一覧にあり、選択されている (オンになっている) ことを確認します。 そうでない場合は、認証するアカウントを選択します。
    - サインインする必要がある場合は、**[Azure にサインイン]** を選択して、認証プロセスを完了します。
...
1. 現在の**スケール ルールの設定**を確認し、最小/最大レプリカ数を次のように構成します。
...
1. [スケーリング] ページで、**[新しいリビジョンとして保存]** を選択します。
...
1. 左側のメニューの [ネットワーク] で、**[イングレス]** を選択します。
